...
# The uploaded asset is always alveoapp-vX.Y.Z-android.apk: the README,
# UpdateService and Obtainium look for that name.
APK_BASENAME="alveoapp"
...
# Asset name for a tag.
apk_name() { printf '%s-%s-android.apk\n' "$APK_BASENAME" "$1"; }
...
main() {
  ...
  local APKSIGNER EXPECTED
  APKSIGNER="$(find_apksigner)"
  [ -x "$APKSIGNER" ] || die "apksigner not found (install build-tools, set ANDROID_HOME)"
  gh repo view --json nameWithOwner >/dev/null || die "gh repo view failed (gh auth login)"
