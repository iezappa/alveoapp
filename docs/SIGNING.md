...
Alveo. The release procedure is in [`RELEASING.md`](RELEASING.md).
...
which deletes every record Alveo keeps on the device — every check-in, journal entry and session note. A lost keystore means
...
```text
~/.android-keystores/alveo/upload-keystore.jks
```

Generate it once, ever:

```bash
mkdir -p ~/.android-keystores/alveo
chmod 700 ~/.android-keystores ~/.android-keystores/alveo
keytool -genkey -v -keystore ~/.android-keystores/alveo/upload-keystore.jks \
        -storetype JKS -keyalg RSA -keysize 2048 -validity 10000 -alias upload
chmod 600 ~/.android-keystores/alveo/upload-keystore.jks
```
...
1. In the password manager, one entry for Alveo with the `.jks` as an
...
keyPassword=...
keyAlias=upload
storeFile=/home/<you>/.android-keystores/alveo/upload-keystore.jks
```
...
```bash
keytool -list -v -keystore ~/.android-keystores/alveo/upload-keystore.jks -alias upload
# "SHA256: AB:CD:..."
```
...
There are none: Alveo never published an APK before release signing existed.
If a debug-signed build ever reaches someone, the way over is: export from
Settings → Your data → Export backup, uninstall, install the release-signed
APK, and import the file. Uninstalling first without exporting loses
everything.
